  ...
  const loadData = async () => {
    try {
      const [postsData, settingsData] = await Promise.all([
        Post.filter({ published: true, category: "thoughts" }, "-created_date"),
        SiteSettings.filter({ page: "thoughts" })
      ]);
      setPosts(postsData);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {posts.length === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="text-2xl font-bold text-gray-800 mb-2">No thoughts yet</h3>
              <p className="text-gray-600">Check back soon for new reflections and insights!</p>
            </div>
          )}
        </div>
      </section>
