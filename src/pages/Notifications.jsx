...
export default function Notifications() {
  ...
  const loadData = async () => {
    try {
      ...
      const userNotifications = await Notification.filter({ user_id: userData.id }, "-created_date");
  ...
  const getNotificationIcon = (type) => {
    const icons = {
      chapter_update: <BookOpen className="w-5 h-5 text-blue-500" />,
      forum_reply: <MessageSquare className="w-5 h-5 text-green-500" />,
      admin_message: <UserIcon className="w-5 h-5 text-purple-500" />,
      story_update: <BookOpen className="w-5 h-5 text-orange-500" />
    };
            ...
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <p className="text-gray-500">
                  You'll receive notifications here when there are new chapters, forum replies, or messages from Anamaria.
                </p>
              </CardContent>
            </Card>
          )}
